...
public class TelaConta {
	...
	public void alterarStatus(long numero, int status) {
		...
		if(status == 2 && conta.getStatus() == 2)
			System.out.println("N�o � poss�vel encerrar a conta\n");
		else if(status == 3) {
			if(conta.getStatus() == 2 || conta.getStatus() == 3)
				System.out.println("N�o � poss�vel bloquear a conta\n");
			else
			...
			if(conta.getStatus() == 1 || conta.getStatus() == 2)
				System.out.println("N�o � poss�vel desbloquear a conta\n");
			else
				conta.setStatus(status);
				repositorioConta.alterar(conta);
		}
	...
	public void buscarConta(long numero) {
		...
		if(conta != null) {
			...
			System.out.println("Esta conta n�o existente\n");
	}
}
